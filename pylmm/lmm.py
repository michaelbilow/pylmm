...
from lmm_helpers import matrixMult
...
class LMM:
    ...
    def __init__(self, Y, K, Kva=None, Kve=None, X0=None, verbose=False):
        ...
        assert np.isclose(K, K.T).all(), "Kinship Matrix should be symmetric"
        ...
        if sum(self.Kva < 1e-6):
            if self.verbose:
                sys.stderr.write("Cleaning {} eigenvalues\n".format(sum(self.Kva < 0)))
    ...
    def transform(self):
        """
         Computes a transformation on the phenotype vector (Yt) and the covariate matrix (X0t).
         The transformation is obtained by left multiplying each parameter by the transpose of the 
         eigenvector matrix of K (the kinship).

         Also sets the parameter q, which is the number of ovariates that are regressed out.
        """
